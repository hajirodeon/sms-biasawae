...
if ($s == "edit")
	...
	//ket
	$qket = mysql_query("SELECT * FROM absensi ".
				"WHERE pbkid = '$kdx'");
	$rket = mysql_fetch_assoc($qket);
	$ket_ket = balikin($rket['ket']);
	}
...
if ($_POST['btnSMP'])
	...
	if (empty($ket))
		...
		{
		//cek
		$qcc = mysql_query("SELECT * FROM absensi ".
					"WHERE pbkID = '$pbkid'");
		...
		if ($tcc != 0)
			{
			//update
			mysql_query("UPDATE absensi SET ket = '$ket' ".
					"WHERE pbkID = '$pbkid'");
			}
		else
			{
			//query
			mysql_query("INSERT INTO absensi(pbkID, ket) VALUES ".
					"('$pbkid', '$ket')");
			}
		...
		$isi_sms = "Ket.Absensi = $ket. untuk NIS:$nis.";
...
if ($s == "edit")
	...
	Ket.
	<br>
	<input name="ket" type="text" value="'.$ket_ket.'" size="30">
	</p>
	...
	if ($total != 0)
		...
		<td width="150"><strong><font color="'.$warnatext.'">Ket.</font></strong></td>
		<td width="50">&nbsp;</td>
		</tr>';
		...
		do
			...
			//ket
			$qdt = mysql_query("SELECT * FROM absensi ".
						"WHERE pbkID = '$i_kd'");
			$rdt = mysql_fetch_assoc($qdt);
			$df_ket = balikin($rdt['ket']);
